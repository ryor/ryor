...
export class Runner implements Runnable
{
  ...
  private next():Promise<void>
  {
    if (this.definitions.length > 0)
    {
      ...
      if (this.context !== undefined && Array.isArray(definition))
        promise = Promise.all((definition as string[]).map((value:RunnablesDefinition):Promise<void> => new Runner([value], this.context).run()))
      ...
      else
      {
        ...
        if (runnable instanceof FunctionRunnable)
          promise = promise
            ...
            {
              if (result !== undefined)
                return new Runner(Array.isArray(result) ? result : [result], runnable.context).run()
            })
      }

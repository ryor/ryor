    ...
    switch (this.command) {
      ...
      default:
        return new Promise<void>((resolve:() => void, reject:(error:string) => void):void => {
          try {
            const childProcess:NodeJS.EventEmitter = spawn(this.command, this.args, { env: process.env, stdio: 'inherit' })
            let error:string = ''
            ...
            reject(`Error encountered spawning child process with command ${chalk.bold(this.command)}: ${error.message}`)
          }
        })
    }
  }

...
export function composeUsageInformationList (items: Map<string, string | undefined> = new Map(), type?: string, minNameLength: number = 0): string {
  const lines: string[] = type !== undefined ? [`${bold(`${composeUsageInformationListItemName(type)}:`)}${LINE_BREAK}`] : []
  ...
  items.forEach((description: string | undefined, name: string): void => {
    lines.push(`${indent}${bold(name)}${name.length < minNameLength ? ' '.repeat(minNameLength - name.length) : ''}    ${description ?? DEFAULT_ITEM_DESCRIPTION}`)
  })

...
import { INDENT, RUNNABLE_USAGE_HEADER, UNRESOLVED_RUNNABLE_ERROR_MESSAGE } from './constants'
import type { RunnableModule, RunnableModuleCommandDefinition } from '../modules'
import type { RunnerConfiguration } from '../runner'

export async function composeRunnableUsageInformation (name: string, configuration: RunnerConfiguration): Promise<string> {
  ...
  let header: string = RUNNABLE_USAGE_HEADER.replace('[ENTRY_DIRECTORY_NAME]', basename(configuration.directory)).replace('[NAME]', name)
  ...
  if (module.commands !== undefined && isPopulatedObject(module.commands)) {
    ...
    header += ' [options]'
    body = composeRunnableArgumentsInformation(module.args)
  }

...
export async function runRunnableModule ({ args: definitions = {}, run }: RunnableModule, name: string, args: string[], runnablesDirectoryPath: string): Promise<void> {
  ...
  if (isValidRunnableSequence(item)) await runRunnableSequence(item as RunnableSequence, runnablesDirectoryPath, name)
}
